...
function updateConfigs (args, newVersion) {
  ...
  args.bumpFiles.forEach(function (bumpFile) {
    ...
    const files = glob.sync(updater.filename, { cwd: process.cwd() })
    if (files.length === 0) {
      console.warn(`No files found matching pattern: ${updater.filename}`)
      return
    }
    ...
    files.forEach((filename) => {
      try {
        ...
        checkpoint(
          args,
          'bumping version in ' + filename + ' from %s to %s',
